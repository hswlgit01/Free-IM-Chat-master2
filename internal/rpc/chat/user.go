...
func (o *chatSvr) checkUpdateInfo(ctx context.Context, req *chat.UpdateUserInfoReq) error {
	if req.AreaCode != nil || req.PhoneNumber != nil {
		if !(req.AreaCode != nil && req.PhoneNumber != nil) {
			log.ZError(ctx, "checkUpdateInfo error", errs.ErrArgs.WrapMsg("areaCode and phoneNumber must be set together"), "req", req)
			return errs.ErrArgs.WrapMsg("areaCode and phoneNumber must be set together")
		}
		if req.AreaCode.Value == "" || req.PhoneNumber.Value == "" {
			if req.AreaCode.Value != req.PhoneNumber.Value {
				log.ZError(ctx, "checkUpdateInfo error", errs.ErrArgs.WrapMsg("area code and phone number must be set together"), "req", req)
				return errs.ErrArgs.WrapMsg("areaCode and phoneNumber must be set together")
			}
		}
	}
	...
	if err != nil {
		log.ZError(ctx, "TakeCredentialsByUserID error", err, "req", req)
		return err
	...
	if req.Account.GetValue() != "" {
		if !stringutil.IsAlphanumeric(req.Account.GetValue()) {
			log.ZError(ctx, "IsAlphanumeric error", err, "req", req)
		...
		if err == nil {
			log.ZError(ctx, "TakeCredentialByAccount error", err, "req", req)
...
func (o *chatSvr) UpdateUserInfo(ctx context.Context, req *chat.UpdateUserInfoReq) (*chat.UpdateUserInfoResp, error) {
	...
	if err != nil {
		log.ZError(ctx, "ToDBAttributeUpdate error", err, "req", req)
		return nil, err
	}
	credUpdate, credDel, err := ToDBCredentialUpdate(req, true)
	if err != nil {
		log.ZError(ctx, "ToDBCredentialUpdate error", err, "req", req)
		return nil, err
	}

	// 打印调试日志
	log.ZInfo(ctx, "准备更新数据库", "userID", req.UserID, "update", update, "updateCount", len(update))

	if len(update) > 0 {
		if err := o.Database.UpdateUseInfo(ctx, req.UserID, update, credUpdate, credDel); err != nil {
			log.ZError(ctx, "UpdateUseInfo error", err, "req", req)
			return nil, err
		}
		log.ZInfo(ctx, "数据库更新成功", "userID", req.UserID)

		// 删除缓存,确保下次查询时能拿到新数据
		cacheKey := fmt.Sprintf("C_ATTRIBUTE_USER_ID:{%s}", req.UserID)
		if _, err := o.redisCli.Del(ctx, cacheKey).Result(); err != nil {
			log.ZWarn(ctx, "删除缓存失败", err, "userID", req.UserID, "cacheKey", cacheKey)
		} else {
			log.ZInfo(ctx, "缓存删除成功", "userID", req.UserID, "cacheKey", cacheKey)
		}

		// 验证:立即查询数据库确认更新成功
		attributes, err := o.Database.FindAttribute(ctx, []string{req.UserID})
		if err != nil {
			log.ZError(ctx, "验证更新失败:查询数据库出错", err, "userID", req.UserID)
		} else if len(attributes) == 0 {
			log.ZError(ctx, "验证更新失败:未找到用户记录", nil, "userID", req.UserID)
		} else {
			log.ZInfo(ctx, "验证更新成功:数据库中的值", "userID", req.UserID,
				"birth_time", attributes[0].BirthTime, "gender", attributes[0].Gender)
		}
	} else {
	...
	// 从组织用户信息中提取 ImServerUserId
	imUserIDs := make([]string, 0, len(orgUsers))
	for _, orgUser := range orgUsers {
		if orgUser.ImServerUserId != "" {
			imUserIDs = append(imUserIDs, orgUser.ImServerUserId)
		}
	}
...
func (o *chatSvr) FindUserPublicInfo(ctx context.Context, req *chat.FindUserPublicInfoReq) (*chat.FindUserPublicInfoResp, error) {
	...
	return &chat.FindUserPublicInfoResp{
		Users: DbToPbAttributes(attributes, isAdmin, opUserID),
	}, nil
}
...
func (o *chatSvr) SearchUserPublicInfo(ctx context.Context, req *chat.SearchUserPublicInfoReq) (*chat.SearchUserPublicInfoResp, error) {
	if req.Keyword == "" {
		return nil, errs.ErrArgs.WrapMsg("keyword is empty")
	}
	opUserID, userType, err := mctx.Check(ctx)
	if err != nil {
		return nil, err
	...
	// 转换AttributeWithOrgUser到Attribute
	attributes := make([]*chatdb.Attribute, len(list))
	for i, userWithOrg := range list {
		attributes[i] = userWithOrg.ToAttribute()
	}
...
func (o *chatSvr) FindUserFullInfo(ctx context.Context, req *chat.FindUserFullInfoReq) (*chat.FindUserFullInfoResp, error) {
	...
	//传入的imUserId查询  转  组织用户信息
	orgUsers, err := o.Database.FindOrgUsersByImServerUserIds(ctx, req.UserIDs)
	if err != nil {
		return nil, err
	}
	...
	// 查询用户属性信息
	log.ZInfo(ctx, "查询用户完整信息", "请求的ImServerUserIds", req.UserIDs, "主账户userIds", userIds)
	attributes, err := o.Database.FindAttribute(ctx, userIds)
	if err != nil {
		return nil, err
	}
	log.ZInfo(ctx, "查询到的用户属性", "数量", len(attributes), "attributes", attributes)

	// 建立 ImServerUserId 到 UserID 的映射
	imUserToUserIdMap := make(map[string]string)
	// 建立 ImServerUserId 到 Role 的映射
	imUserToRoleMap := make(map[string]string)
	for _, orgUser := range orgUsers {
		if orgUser.ImServerUserId != "" && orgUser.UserId != "" {
			imUserToUserIdMap[orgUser.ImServerUserId] = orgUser.UserId
			imUserToRoleMap[orgUser.ImServerUserId] = string(orgUser.Role)
		}
	}

	// 转换为协议缓冲区格式，并保持与请求 UserIDs 的对应关系
	users := DbToPbUserFullInfosWithMapping(attributes, req.UserIDs, imUserToUserIdMap, imUserToRoleMap, isAdmin, opUserID)
...
func (o *chatSvr) SearchUserFullInfo(ctx context.Context, req *chat.SearchUserFullInfoReq) (*chat.SearchUserFullInfoResp, error) {
	...
	if err != nil {
		log.ZError(ctx, "SearchUserFullInfo", err, "get org id failed")
	}
	total, list, err := o.Database.Search(ctx, req.Normal, req.Keyword, req.Genders, req.Pagination, orgId)
	if err != nil {
		return nil, err
	}
	return &chat.SearchUserFullInfoResp{
		Total: uint32(total),
		Users: DbToPbUserFullInfosByOrg(list, isAdmin, opUserID),
	}, nil
}
...
func (o *chatSvr) SearchUserInfo(ctx context.Context, req *chat.SearchUserInfoReq) (*chat.SearchUserInfoResp, error) {
	...
	return &chat.SearchUserInfoResp{
		Total: uint32(total),
		Users: DbToPbUserFullInfos(list, isAdmin, opUserID),
	}, nil
}
...
func (o *chatSvr) CheckUserExist(ctx context.Context, req *chat.CheckUserExistReq) (resp *chat.CheckUserExistResp, err error) {
	...
	if req.User.PhoneNumber != "" {
		...
		if account != nil {
			log.ZDebug(ctx, "Check Number is", "account", account.Account)
			log.ZDebug(ctx, "Check userID is", "userID", account.UserID)
	...
	if req.User.Email != "" {
		...
		if account != nil {
			log.ZDebug(ctx, "Check email is", "email", account.Account)
			log.ZDebug(ctx, "Check userID is", "userID", account.UserID)
			return &chat.CheckUserExistResp{Userid: account.UserID, IsRegistered: true}, nil
		}
	}
	if req.User.Account != "" {
		...
		if account != nil {
			log.ZDebug(ctx, "Check account is", "account", account.Account)
			log.ZDebug(ctx, "Check userID is", "userID", account.UserID)
			return &chat.CheckUserExistResp{Userid: account.UserID, IsRegistered: true}, nil
		}
	}
