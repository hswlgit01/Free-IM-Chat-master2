...
func (o *chatSvr) SendVerifyCode(ctx context.Context, req *chat.SendVerifyCodeReq) (*chat.SendVerifyCodeResp, error) {
	...
	case constant.VerificationCodeForLogin, constant.VerificationCodeForResetPassword, constant.VerificationCodeForPaymentPwd:
		if req.Email == "" {
	...
	case constant.VerificationCodeForResetEmail:
		if req.Email != "" {
			if err := chat.EmailCheck(req.Email); err != nil {
				return nil, errs.ErrArgs.WrapMsg("email must be right")
			}
		} else {
			return nil, errs.ErrArgs.WrapMsg("email is empty")
		}
	default:
	...
	if req.AreaCode != "" {
		...
		case constant.VerifyWeiwebs:
		case constant.VerifySmart:
		default:
	...
	if isEmail {
		...
		// 检查是否为中国大陆号码（+86）
		isMainlandChina := strings.TrimPrefix(req.AreaCode, "+") == "86"

		sendCode = func() error {
			// 根据区号选择不同的短信服务
			if isMainlandChina && o.conf.Phone.Use != constant.VerifySuperCode {
				// 中国大陆使用weiwebs
				weiweb := sms.NewWeiwebs(
					o.conf.Phone.Weiwebs.Account,
					o.conf.Phone.Weiwebs.Password,
					o.conf.Phone.Weiwebs.SignName,
				)
				return weiweb.SendCode(ctx, req.AreaCode, req.PhoneNumber, code)
			} else {
				// 其他地区使用阿里云或已配置的默认服务
...
func (o *chatSvr) verifyCode(ctx context.Context, account string, verifyCode string, type_ verifyType, usedFor int32) (string, error) {
	if verifyCode == "" {
	...
	last, err := o.Database.TakeLastVerifyCode(ctx, account, usedFor)
	if err != nil {
		if dbutil.IsDBNotFound(err) {
...
func (o *chatSvr) VerifyCode(ctx context.Context, req *chat.VerifyCodeReq) (*chat.VerifyCodeResp, error) {
	...
	if req.PhoneNumber != "" {
		account = o.verifyCodeJoin(req.AreaCode, req.PhoneNumber)
		codeID, err = o.verifyCode(ctx, account, req.VerifyCode, phone, req.UsedFor)
		if err != nil {
			return nil, err
		}
	} else {
		account = req.Email
		codeID, err = o.verifyCode(ctx, account, req.VerifyCode, mail, req.UsedFor)
		if err != nil {
			return nil, err
		}
	}

	if req.DeleteAfterVerify && codeID != "" {
		if err := o.Database.DelVerifyCode(ctx, codeID); err != nil {
			log.ZWarn(ctx, "failed to delete verify code after verification", err, "codeID", codeID)
		}
	}
...
func (o *chatSvr) RegisterUser(ctx context.Context, req *chat.RegisterUserReq) (*chat.RegisterUserResp, error) {
	...
	if !isAdmin {
		...
		if req.User.Email == "" {
			verifyCodeID, err = o.verifyCode(ctx, o.verifyCodeJoin(req.User.AreaCode, req.User.PhoneNumber), req.VerifyCode, phone, constant.VerificationCodeForRegister)
			if err != nil {
				return nil, err
			}
		} else {
			verifyCodeID, err = o.verifyCode(ctx, req.User.Email, req.VerifyCode, mail, constant.VerificationCodeForRegister)
			if err != nil {
				return nil, err
			}
		}
	...
	// 注册成功后，如果验证了验证码，则删除验证码
	if verifyCodeID != "" {
		if err := o.Database.DelVerifyCode(ctx, verifyCodeID); err != nil {
			log.ZWarn(ctx, "failed to delete verify code after registration", err, "codeID", verifyCodeID)
			// 不影响注册流程，只记录日志
		}
	}
...
func (o *chatSvr) Login(ctx context.Context, req *chat.LoginReq) (*chat.LoginResp, error) {
	resp := &chat.LoginResp{}
	log.ZInfo(ctx, "Login", "req.Password", req.Password, "req.VerifyCode", req.VerifyCode, "req.Account", req.Account, "req.Email", req.Email, "req.PhoneNumber", req.PhoneNumber)
	...
	if err != nil {
		...
		log.ZInfo(ctx, "TakeCredentialByAccount error")
		return nil, err
	}
	...
	if req.Password == "" {
		...
		if req.Email == "" {
			account := o.verifyCodeJoin(req.AreaCode, req.PhoneNumber)
			id, err = o.verifyCode(ctx, account, req.VerifyCode, phone, constant.VerificationCodeForLogin)
			if err != nil {
				return nil, err
			}
		} else {
			account := req.Email
			id, err = o.verifyCode(ctx, account, req.VerifyCode, mail, constant.VerificationCodeForLogin)
			if err != nil {
				return nil, err
			}
		}
	...
	o.redisCli.Del(ctx, fmt.Sprintf("C_LOGIN_RECORD_USER_ID:{%s}", credential.UserID))
