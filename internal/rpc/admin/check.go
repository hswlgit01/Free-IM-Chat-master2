...
func (o *adminServer) CheckRegisterForbidden(ctx context.Context, req *admin.CheckRegisterForbiddenReq) (*admin.CheckRegisterForbiddenResp, error) {
	...
	for _, forbidden := range forbiddens {
		if forbidden.LimitRegister {
			return nil, eerrs.ErrForbidden.WrapMsg("ip forbidden for register")
		}
	}
...
func (o *adminServer) CheckLoginForbidden(ctx context.Context, req *admin.CheckLoginForbiddenReq) (*admin.CheckLoginForbiddenResp, error) {
	...
	if _, err := o.Database.GetBlockInfo(ctx, req.UserID); err == nil {
		return nil, eerrs.ErrForbidden.WrapMsg("user blocked")
	} else if !dbutil.IsDBNotFound(err) {
		return nil, err
	}
	// 检查超管封禁 - 使用与其他封禁完全相同的方式
	if _, err := o.Database.GetSuperAdminForbidden(ctx, req.UserID); err == nil {
		return nil, eerrs.ErrForbidden.WrapMsg("user forbidden by super admin")
	} else if !dbutil.IsDBNotFound(err) {
		return nil, err
	}
