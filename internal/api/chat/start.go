...
import (
	...
	"github.com/openimsdk/chat/freechat"
...
func Start(ctx context.Context, index int, cfg *Config) error {
	...
	engine := gin.New()
	engine.Use(gin.Recovery(), mw.CorsHandler(), middleware.GinParseOperationID())
	SetChatRoute(engine, adminApi, mwApi)

	/*
		二次开发内容
	*/
	freechat.RegisterChatExtension(&plugin.ChatConfig{
		ApiConfig:     cfg.ApiConfig,
		Discovery:     cfg.Discovery,
		Share:         cfg.Share,
		MongodbConfig: cfg.MongodbConfig,
		RedisConfig:   cfg.RedisConfig,
		ChatRpcConfig: cfg.ChatRpcConfig,
		RuntimeEnv:    cfg.RuntimeEnv,
	}, engine, chatClient, adminClient, im)
	log.ZInfo(context.TODO(), "RegisterChatExtension success")
...
func SetChatRoute(router gin.IRouter, chat *Api, mw *chatmw.MW) {
	...
	account.POST("/login", chat.Login)                                   // EmbedLogin
	...
	//decryptMiddleware := depmw.DecryptMiddleware()
	//account.POST("/crypto/register", mw.CheckAdminOrNil, decryptMiddleware, chat.RegisterUser)     // crypto Register
	//account.POST("/crypto/login", decryptMiddleware, chat.EmbedLogin)                                   // crypto EmbedLogin
	//account.POST("/crypto/password/reset", decryptMiddleware, chat.ResetPassword)                  // crypto Forgot password
	//account.POST("/crypto/password/change", decryptMiddleware, mw.CheckToken, chat.ChangePassword) // crypto Change password
