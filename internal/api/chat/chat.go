...
func (o *Api) ChangePassword(c *gin.Context) {
	...
	if resp.ShouldKickOut {
		imToken, err := o.imApiCaller.ImAdminTokenWithDefaultAdmin(c)
		if err != nil {
			apiresp.GinError(c, err)
			return
		}
		err = o.imApiCaller.ForceOffLine(mctx.WithApiToken(c, imToken), resp.ImServerID)
		if err != nil {
			apiresp.GinError(c, err)
			return
		}
...
func (o *Api) UpdateUserInfo(c *gin.Context) {
	...
	// 批量更新用户的IM账户信息，全部更新
	for i := range respUpdate.ImUserIDs {
		err = o.imApiCaller.UpdateUserInfo(mctx.WithApiToken(c, imToken), respUpdate.ImUserIDs[i], nickName, faceURL)
		if err != nil {
			apiresp.GinError(c, err)
			return
		}
