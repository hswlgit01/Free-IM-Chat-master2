...
func SetAdminRoute(router gin.IRouter, admin *Api, mw *chatmw.MW, cfg *Config, client discovery.SvcDiscoveryRegistry) {
	...
	//adminRouterGroup.POST("/login", admin.AdminLogin)                                   // EmbedLogin
	...
	adminRouterGroup.POST("/change_org_user_password", mw.CheckToken, admin.ChangeOrgUserPassword) // Change organization user's password
	...
	// 补偿金管理相关路由
	{
		compensationRouter := router.Group("/wallet/compensation")
		compensationRouter.POST("/get_settings", mw.CheckAdmin, admin.GetCompensationSettings)
		compensationRouter.POST("/update_settings", mw.CheckAdmin, admin.UpdateCompensationSettings)
		compensationRouter.POST("/get_user_balance", mw.CheckAdmin, admin.GetUserCompensationBalance)
		compensationRouter.POST("/adjust_user_balance", mw.CheckAdmin, admin.AdjustUserCompensationBalance)
	}
}
