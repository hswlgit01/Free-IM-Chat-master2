...
import (
	...
	"github.com/openimsdk/chat/pkg/constant"
...
// ChangeOrgUserPassword 组织管理员修改组织内某个用户的登录密码。
// 和 ChangeAdminPassword 保持一致：API 层仅转发，具体逻辑在 RPC 服务层实现。
func (o *Api) ChangeOrgUserPassword(c *gin.Context) {
	log.ZInfo(c, "ChangeOrgUserPassword request received")
	a2r.Call(c, chat.ChatClient.ChangePassword, o.chatClient)
}
...
func (o *Api) ImportUserByXlsx(c *gin.Context) {
	...
	if err := xlsx.ParseAll(file, &users); err != nil {
		apiresp.GinError(c, errs.ErrArgs.WrapMsg("xlsx file parse freeErrors "+err.Error()))
		return
	}
...
func (o *Api) xlsx2user(users []model.User) ([]*chat.RegisterUserInfo, error) {
	...
	for i, info := range users {
		...
		if !strings.HasPrefix(info.AreaCode, "+") {
			return nil, errs.ErrArgs.WrapMsg("areaCode format freeErrors")
		}
