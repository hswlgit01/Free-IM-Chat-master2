...
import (
	...
	"github.com/openimsdk/chat/tools/db/tx"
...
type ChatDatabaseInterface interface {
	...
	TakeLastVerifyCode(ctx context.Context, account string, usedFor int32) (*chatdb.VerifyCode, error)
	Search(ctx context.Context, normalUser int32, keyword string, gender int32, pagination pagination.Pagination, org_id string) (int64, []*chatdb.AttributeWithOrgUser, error)
	...
	FindOrgUsersByImServerUserIds(ctx context.Context, imServerUserIds []string) ([]*chatdb.OrganizationUser, error)
	FindOrgUserByUserIds(ctx context.Context, userIds []string) ([]*chatdb.OrganizationUser, error)
	GetImServerID(ctx context.Context, userID, orgid string) (*chatdb.OrganizationUser, error)
}
...
func NewChatDatabase(cli *mongoutil.Client) (ChatDatabaseInterface, error) {
	...
	organizationUser, err := chat.NewOrganizationUser(cli.GetDB())
	if err != nil {
		return nil, err
	}
	return &ChatDatabase{
...
type ChatDatabase struct {
	...
	organizationUser chatdb.OrganizationUserInterface
}
...
func (o *ChatDatabase) TakeLastVerifyCode(ctx context.Context, account string, usedFor int32) (*chatdb.VerifyCode, error) {
	return o.verifyCode.TakeLast(ctx, account, usedFor)
}
...
func (o *ChatDatabase) Search(ctx context.Context, normalUser int32, keyword string, gender int32, pagination pagination.Pagination, org_id string) (int64, []*chatdb.AttributeWithOrgUser, error) {
	...
	total, totalUserWithOrg, err := o.attribute.SearchNormalUser(ctx, keyword, forbiddenIDs, gender, pagination, org_id)
	if err != nil {
		return 0, nil, err
	}
	return total, totalUserWithOrg, nil
}
...
func (o *ChatDatabase) FindOrgUsersByImServerUserIds(ctx context.Context, imServerUserIds []string) ([]*chatdb.OrganizationUser, error) {
	if len(imServerUserIds) == 0 {
		return []*chatdb.OrganizationUser{}, nil
	}

	orgUsers, err := o.organizationUser.FindByImServerUserIds(ctx, imServerUserIds)
	if err != nil {
		return nil, err
	}
...
func (o *ChatDatabase) FindOrgUserByUserIds(ctx context.Context, userIds []string) ([]*chatdb.OrganizationUser, error) {
	orgUsers, err := o.organizationUser.Find(ctx, userIds)
	if err != nil {
		return nil, err
	}
	return orgUsers, nil
...
func (o *ChatDatabase) GetImServerID(ctx context.Context, userID, orgid string) (*chatdb.OrganizationUser, error) {
	orgUser, err := o.organizationUser.TakeByOrgid(ctx, userID, orgid)
	if err != nil {
		return nil, err
	}
	return orgUser, nil
