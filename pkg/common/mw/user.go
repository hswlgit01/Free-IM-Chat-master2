...
func AddUserType() grpc.DialOption {
	return grpc.WithChainUnaryInterceptor(func(ctx context.Context, method string, req, reply interface{}, cc *grpc.ClientConn, invoker grpc.UnaryInvoker, opts ...grpc.CallOption) error {
		...
		// 处理OrgId
		if orgIdArr, _ := ctx.Value(constant.RpcOrgId).([]string); len(orgIdArr) > 0 {
			log.ZInfo(ctx, "add org id", "method", method, "orgId", orgIdArr[0])
			ctx = context.WithValue(ctx, constant.RpcCustomHeader, append(headers, constant.RpcOrgId))
			ctx = context.WithValue(ctx, constant.RpcOrgId, orgIdArr)
		}
