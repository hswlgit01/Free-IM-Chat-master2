...
// GetOrgId 从上下文中获取组织ID
func GetOrgId(ctx context.Context) (string, error) {
	orgIdArr, ok := ctx.Value(constant.RpcOrgId).([]string)
	if !ok || len(orgIdArr) == 0 {
		return "", errs.ErrNoPermission.WrapMsg("org id empty")
	}
	return orgIdArr[0], nil
}
