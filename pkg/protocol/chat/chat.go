...
func (x *SendVerifyCodeReq) Check() error {
	if x.UsedFor < constant.VerificationCodeForRegister || x.UsedFor > constant.VerificationCodeForResetEmail {
...
func (x *VerifyCodeReq) Check() error {
	...
	// deleteAfterVerify为可选参数，默认为false，表示验证成功后不删除验证码
	return nil
}
...
func (x *RegisterUserReq) Check() error {
	...
	if x.Platform < constantpb.IOSPlatformID || x.Platform > constantpb.H5WebPlatformID {
